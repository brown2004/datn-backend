...
func (h *PCAgentHandler) HandleUpdateOwnProtection(w http.ResponseWriter, r *http.Request) {
	var req struct {
		PCAgentID   string `json:"pc_agent_id"`
		AgentSecret string `json:"agent_secret"`
		Enabled     bool   `json:"enabled"`
	...
	agent, err := h.pairing.UpdatePCAgentOwnProtection(r.Context(), domain.UpdatePCAgentOwnProtectionInput{
		PCAgentID:   req.PCAgentID,
		AgentSecret: req.AgentSecret,
		Enabled:     req.Enabled,
	})
	if err != nil {
		switch {
		case errors.Is(err, usecase.ErrAgentCredentialInvalid):
			writeMessageError(w, http.StatusUnauthorized, "ThĂ´ng tin xĂ¡c thá»±c thiáº¿t bá»‹ khĂ´ng há»£p lá»‡")
		default:
			writeInternalError(w, err)
		}
		return
